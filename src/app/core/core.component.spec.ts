...
function classItemTest()
{
    ...
    {
        expect( Number(itmObj.quantity) ).toBe( itmQty )
    }) ); 
}
...
function classIngredientTest()
{
    ...
    it( 'adds Ingredient quantity ' + addQty, () => 
    {
        ...
        expect( Number(ingrObj.quantity) ).toBe( ingrQty + addQty )
    ...
    it( 'subtracts Ingredient quantity ' + subtQty, () => 
    {
        ...
        expect( Number(ingrObj.quantity) ).toBe( ingrQty + addQty - subtQty )
    } );
}
...
function classFridgeTest()
{
    ...
    it( 'adds quantity of Item to the Fridge', async( () => 
    {
        for ( var i = 0; i < itms.length; i++ )
        {
            ...
            expect( Number(frgItm.quantity) ).toBe( itmQty[i] + toAddQty )
        }
    } ) );
    ...
    it( 'removes quantity of Item from the Fridge', async( () => 
    {
        for ( var i = 0; i < itms.length; i++ )
        {
            ...
            expect( Number(frgItm.quantity) ).toBe( itmQty[i] + toAddQty - toRmvQty )
        }
    } ) );
